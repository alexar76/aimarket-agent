...
class AIMarketAgent:
    ...
    def __init__(
        ...
        # Ed25519 key of the party that SIGNED it — the hub for a capability it serves
        # itself, the provider for a federated one. Failures are surfaced in the result
        # as receipt_verified / receipt_verify_reason, never raised.
        self.verify_receipts = verify_receipts
        self.session = httpx.Client(timeout=timeout)
        # Keys are resolved per origin and cached. Before 2.2.0 there was a single
        # verifier built from the hub's well-known document and applied to everything,
        # which reported invalid-signature for every federated capability — 42 of the 47
        # on modelmarket.dev — on receipts that were valid.
        # A lambda, not `self.session`. `.session` is a public, replaceable attribute — the
        # only way a caller can inject an instrumented, retrying or mock client — and passing
        # the object bound it permanently, so a swap left key lookups on the discarded client
        # and every receipt came back `receipt_verified: False, no-origin-key`.
    ...
    def run(self, task: str, verify: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        # No verifier is built here any more: the key depends on which capability each
        # step routes to, so it is resolved per step from that step's source_hub.
        ...
        for step in plan:
            ...
            # Verify against the key of the capability's ORIGIN, not the hub's: a
            # federated step's receipt is signed by whoever ran it.
            if self.verify_receipts:
                vr = self._verifiers.verify(body.get("receipt"), source_hub=source_hub)
    ...
    def invoke_single(
        ...
        if self.verify_receipts and isinstance(body, dict) and body.get("receipt"):
            vr = self.verify_receipt(body.get("receipt"), source_hub=source_hub)
            body["receipt_verified"] = bool(vr)
            body["receipt_verify_reason"] = vr.reason
    ...
    def verify_receipt(self, receipt: dict[str, Any], source_hub: str = ""):
        """Verify a single receipt against the key of the party that signed it.

        Pass *source_hub* — the value the catalogue reported for the capability — to
        check a FEDERATED receipt. Omit it (or pass ``"local"``) for one the hub served
        itself. The distinction is not cosmetic: a federated receipt carries the
        provider's signature, so checking it against the hub's key fails on a valid
        receipt. The parameter is optional so existing calls keep working; they simply
        keep asking about the hub.

        Keys are fetched once per origin and cached. Returns a ``receipts.VerifyResult``
        (truthy when verified).
        """
        return self._verifiers.verify(receipt, source_hub=source_hub)
